...
export const metadata = {
  title: "التسويق",
  description: "كتاب التسويق - 14 فصل، 420 سؤال، 350 بطاقة",
};

const totalQuestions = marketingBook.chapters.reduce((s, c) => s + c.quiz.length, 0);
const totalFlashcards = marketingBook.chapters.reduce((s, c) => s + c.flashcards.length, 0);

export default function MarketingBookPage() {
  return (
    <>
      <Nav />
      <main className="mx-auto max-w-5xl px-4 sm:px-6 py-12 sm:py-16">
        <nav className="flex items-center gap-2 text-sm text-muted mb-6">
          ...
          <ArrowLeft className="size-3" />
          <span className="text-foreground font-medium">التسويق</span>
        </nav>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-royal-700 via-royal-600 to-cyan-600 p-8 sm:p-12 text-white shadow-card">
          ...
          <div className="relative">
            ...
            <div className="mt-8 grid grid-cols-3 gap-4 max-w-md">
              <div>
                <div className="text-2xl font-black">{marketingBook.chapters.length}</div>
        ...
        <div className="mt-8">
          <BookProgressSummary
            bookSlug="marketing"
            totalChapters={marketingBook.chapters.length}
          />
        </div>
        ...
        <div id="chapters" className="mt-10 flex items-center justify-between mb-6 scroll-mt-20">
          ...
          <div className="hidden sm:inline-flex items-center gap-1.5 text-xs font-bold text-cyan-700 bg-cyan-50 rounded-full px-3 py-1 ring-1 ring-cyan-200">
            ...
            {marketingBook.chapters.length} فصل متاح
          </div>
        </div>

        <div className="grid gap-3 sm:gap-4 sm:grid-cols-2">
          {marketingBook.chapters.map((ch) => (
            <Link
              key={ch.slug}
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <ChapterProgress bookSlug="marketing" chapterNum={ch.num} />
                </div>
